 ...
 $destination = "C:\ISOs\OS\Clients\Win11\21H2"
 $WIMDestination = "$destination\Windows11Enterprise-21H2.wim"
 $Drives = Get-CimInstance Win32_LogicalDisk | Where-Object -Property VolumeName -eq "CCSA_X64FRE_EN-US_DV9" 
 $OSISO = ($Drives).DeviceID
 $WIMDir = "C:\ISOs\OS\Clients\Win11\21H2\WIM"
 $WIMDirTest = Test-Path -Path "C:\ISOs\OS\Clients\Win11\21H2\WIM" -IsValid
 If($WIMDirTest -eq $true){
 Copy-Item -Path "$OSISO\*" -Destination $destination -Recurse
 $sourceWIM = "C:\ISOs\OS\Clients\Win11\21H2\sources\install.wim"
 Export-WindowsImage -SourceImagePath "C:\ISOs\OS\Clients\Win11\21H2\sources\install.wim" -SourceIndex 6  -DestinationName 'Windows 11 Enterprise 21H2' -DestinationImagePath $WIMDestination
 }
 ElseIf($WIMDirTest -eq $false){
 New-Item -Path "C:\ISOs\OS\Clients\Win11\21H2\WIM" -ItemType Directory -Force
 Copy-Item -Path "$OSISO\*" -Destination $destination -Recurse
 $sourceWIM = "C:\ISOs\OS\Clients\Win11\21H2\sources\install.wim"
 Export-WindowsImage -SourceImagePath "C:\ISOs\OS\Clients\Win11\21H2\sources\install.wim" -SourceIndex 6  -DestinationName 'Windows 11 Enterprise 21H2' -DestinationImagePath $WIMDestination
 }
